...
export const ${hookName} = ${pure} ${functionName}({ ${innerContent}, functionName: '${item.name}' })`);
                ...
                if (hasWriteFunction) {
                    {
                        const actionName = getActionName(config, actionNames, 'write', contract.name);
...
export const ${actionName} = ${pure} ${functionName}({ ${innerContent} })`);
                        ...
                        for (const item of writeItems) {
                            ...
                            if (names.has(item.name))
                                continue;
                            names.add(item.name);
                            const actionName = getActionName(config, actionNames, 'write', contract.name, item.name);
                            const docString = genDocString('writeContract', contract, {
                                name: 'functionName',
                                value: item.name,
                            });
...
export const ${actionName} = ${pure} ${functionName}({ ${innerContent}, functionName: '${item.name}' })`);
                    ...
                    {
                        const actionName = getActionName(config, actionNames, 'simulate', contract.name);
...
export const ${actionName} = ${pure} ${functionName}({ ${innerContent} })`);
                        ...
                        for (const item of writeItems) {
                            ...
                            if (names.has(item.name))
                                continue;
                            names.add(item.name);
                            const actionName = getActionName(config, actionNames, 'simulate', contract.name, item.name);
                            const docString = genDocString('simulateContract', contract, {
                                name: 'functionName',
                                value: item.name,
                            });
...
export const ${actionName} = ${pure} ${functionName}({ ${innerContent}, functionName: '${item.name}' })`);
                ...
                if (hasEvent) {
                    const actionName = getActionName(config, actionNames, 'watch', contract.name);
...
export const ${actionName} = ${pure} ${functionName}({ ${innerContent} })`);
                    ...
                    for (const item of eventItems) {
                        ...
                        if (names.has(item.name))
                            continue;
                        names.add(item.name);
                        const actionName = getActionName(config, actionNames, 'watch', contract.name, item.name);
                        const docString = genDocString('watchContractEvent', contract, {
                            name: 'eventName',
                            value: item.name,
                        });
...
function genDocString(actionName, contract, item) {
...
function getActionName(config, actionNames, type, contractName, itemName) {
    ...
    if (typeof config.getActionName === 'function')
        ...
        if (type === 'watch')
            actionName = `${actionName}Event`;
    }
    if (actionNames.has(actionName))
        throw new Error(`Action name "${actionName}" must be unique for contract "${contractName}". Try using \`getActionName\` to create a unique name.`);
